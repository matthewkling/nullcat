...
* New feature: weighted pair sampling via `wt_row` and `wt_col` parameters.
  ...
  diagnostics, and vegan integration. Supplying both `wt_row` and `wt_col`
  produces a Gibbs-like alternating scheme. Uses Vose's alias method for O(1)
  weighted pair sampling with negligible performance overhead (<5%).
