...
Markov chain in the space of numeric community matrices. Internally,
each simulation "step" advances the chain by re-applying
[`quantize()`](https://matthewkling.github.io/nullcat/reference/quantize.md)
to the current matrix using the settings provided via `...`.
...
- ...:
  ...
  `fixed`, `method`, `n_iter`, etc. Do **not** supply `x` or `n_iter`
  here; `x` is provided by vegan and `n_iter` is set internally from
  `thin`. See
  [`quantize()`](https://matthewkling.github.io/nullcat/reference/quantize.md)
  for details.
